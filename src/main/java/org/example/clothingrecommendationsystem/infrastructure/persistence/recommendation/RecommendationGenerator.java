...
public class RecommendationGenerator implements IRecommendationGenerator {
    ...
    public Recommendation generateRecommendation(@NonNull String prompt,
        ...
        PieceOfClothes single = pickBest(byCat.get(PieceOfClothes.PieceCategory.Single), req, true, true, now, palette);
        if (single != null && RNG.nextDouble() < 0.5) addAndLearn(outfit, single, palette);

        if (outfit.isEmpty()) {
            buildInnerLayers(byCat.get(PieceOfClothes.PieceCategory.Innerlayer), outfit, req, w.getFeelsLikeTemperature(), now, palette);
            addBestUnique(outfit, byCat.get(PieceOfClothes.PieceCategory.Bottom), req, true, true, now, palette);
        }
        ...
        if (outers != null && !outers.isEmpty()) {
            if (needOuter) {
                addBestUnique(outfit, outers, req, true, true, now, palette);
                ...
                List<PieceOfClothes> strict = outers.stream()
                        ...
                        .toList();
                addBestUnique(outfit, strict, req, true, true, now, palette);
            }
        }

        maybeAddAccessories(byCat.get(PieceOfClothes.PieceCategory.Accessories), outfit, precip, now, req, palette);
        maybeAddHeadwear(byCat.get(PieceOfClothes.PieceCategory.Headwear), outfit, precip, w.getFeelsLikeTemperature(), now, req, palette);
    ...
    private Map<PieceOfClothes.PieceCategory, List<PieceOfClothes>> buildTempBuckets(List<PieceOfClothes> wardrobe, PieceOfClothes.TemperatureCategory now) {
        ...
        while (byCat.values().stream().allMatch(List::isEmpty) && radius <= 5) {
            ...
            byCat = wardrobe.stream().filter(Objects::nonNull).filter(tempOk)
                    .collect(Collectors.groupingBy(PieceOfClothes::getPieceCategory, () -> new EnumMap<>(PieceOfClothes.PieceCategory.class), Collectors.toList()));
            radius++;
        }
        return byCat;
    }

    private void buildInnerLayers(List<PieceOfClothes> inners, List<PieceOfClothes> outfit,
                                  ParsedRequest req, float t, PieceOfClothes.TemperatureCategory now, Set<String> palette) {
        ...
        List<PieceOfClothes> basePool = new ArrayList<>(tier1);
        basePool.addAll(tier2);
    ...
    private void maybeAddAccessories(List<PieceOfClothes> acc, List<PieceOfClothes> outfit,
                                     boolean precip, PieceOfClothes.TemperatureCategory now, ParsedRequest req, Set<String> palette) {
    ...
    private void maybeAddHeadwear(List<PieceOfClothes> head, List<PieceOfClothes> outfit,
                                  boolean precip, float t, PieceOfClothes.TemperatureCategory now, ParsedRequest req, Set<String> palette) {
        if (head == null || head.isEmpty()) return;
        boolean must = precip || t < 10;
        if (RNG.nextDouble() < (must ? 1.0 : 0.5)) addAndLearn(outfit, head.get(RNG.nextInt(head.size())), palette);
    }

    private ParsedRequest ensureRandom(ParsedRequest r) {
        PieceOfClothes.Style s = r.style == null ? List.copyOf(UA_STYLE_MAP.values()).get(RNG.nextInt(UA_STYLE_MAP.size())) : r.style;
        String col = r.color == null ? new ArrayList<>(BASE_PALETTE.keySet()).get(RNG.nextInt(BASE_PALETTE.size())) : r.color;
        return new ParsedRequest(s, col);
    }

    private ParsedRequest parseGenerationCharacteristics(String txt) {
        PieceOfClothes.Style style = UA_STYLE_MAP.entrySet().stream().filter(e -> txt.contains(e.getKey())).map(Map.Entry::getValue).findFirst().orElse(null);
        String col = SHADE_TO_BASE.entrySet().stream().filter(e -> txt.contains(e.getKey())).map(Map.Entry::getValue).findFirst().orElse(null);
    ...
    private PieceOfClothes pickBest(List<PieceOfClothes> items, ParsedRequest req, boolean strictStyle, boolean strictColor,
        ...
        for (PieceOfClothes pc : items) {
            ...
            if (req.color != null) {
                ...
                if (!exact) {
                    boolean near = COMPATIBLE_BASE.getOrDefault(req.color, List.of()).stream().map(BASE_PALETTE::get).filter(Objects::nonNull)
                            .anyMatch(lst -> lst.stream().anyMatch(all::contains));
                    score += near ? 50 : (strictColor ? 150 : 15);
                }
            }
            if (!palette.isEmpty()) {
                boolean comp = palette.stream().anyMatch(c -> collectColors(pc).stream().anyMatch(cl -> cl.equals(c) || COMPATIBLE_BASE.getOrDefault(c, List.of()).contains(cl)));
                if (!comp) score += 100;
            }
    ...
    private Set<String> collectColors(PieceOfClothes pc) {
        ...
        Optional.ofNullable(pc.getColor()).ifPresent(c -> s.add(SHADE_TO_BASE.getOrDefault(c.toLowerCase(), c.toLowerCase())));
        Optional.ofNullable(pc.getCharacteristics()).orElse(List.of()).forEach(ch -> {
    ...
    private static boolean addUnique(List<PieceOfClothes> outfit, PieceOfClothes cand) {
        ...
        boolean exists = outfit.stream().anyMatch(p -> Optional.ofNullable(p.getName()).orElse("").trim().toLowerCase().contains(n) || n.contains(Optional.ofNullable(p.getName()).orElse("").trim().toLowerCase()));
